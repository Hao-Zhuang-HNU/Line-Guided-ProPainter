...
class Trainer:
    ...
    def _sobel_grad(self, x):
        """Return Sobel gradient magnitude for [B,T,3,H,W]."""
        b, t, c, h, w = x.shape
    ...
    def load(self):
        ...
        if latest_epoch is not None:
            ...
            missing, unexpected = self.netG.load_state_dict(dataG, strict=False)
            if self.config['global_rank'] == 0 and len(missing) > 0:
                print(f'[Line-Guided ProPainter] missing Gen-Net keys: {missing}')
            if self.config['global_rank'] == 0 and len(unexpected) > 0:
                print(f'[Line-Guided ProPainter] unexpected Gen-Net keys: {unexpected}')
    ...
    def _train_epoch(self, pbar):
        ...
        while train_data is not None:
            ...
            # ---- image propagation ----
            netG_core = self.netG.module if hasattr(self.netG, 'module') else self.netG
            prop_imgs, updated_local_masks = netG_core.img_propagation(masked_local_frames, pred_flows_bi, local_masks, interpolation=self.interp_mode)
            ...
            # line-weighted reconstruction loss around pkl-rendered wireframe regions
            line_weighted_weight = self.config['losses'].get('line_weighted_weight', 0)
            if self.use_line_guidance and line_weighted_weight > 0:
                ...
                line_weighted_loss = torch.mean(torch.abs(pred_imgs - frames) * masks * line_weight) / denom
                line_weighted_loss = line_weighted_loss * line_weighted_weight
                gen_loss += line_weighted_loss
                self.add_summary(self.gen_writer, 'loss/line_weighted_loss', line_weighted_loss.item())

            # Sobel structure loss in the hole region, optionally emphasized around line regions
            sobel_weight = self.config['losses'].get('sobel_weight', 0)
            if self.use_line_guidance and sobel_weight > 0:
                ...
                sobel_mask = masks * (1.0 + sobel_line_alpha * line_region)
                denom = torch.mean(sobel_mask).clamp_min(1e-6)
                sobel_loss = torch.mean(torch.abs(sobel_pred - sobel_gt) * sobel_mask) / denom
